			...
			if (request_type.equals("register")) {
				...
				if ((addr == null || addr.trim().length() == 0) || (email == null || email.trim().length() == 0)
					...
					if (terms == null || !terms.equals("on")) {
						resp.sendRedirect(
								"register.jsp?msg=You must accept the Terms and conditions before Registration.");
					} else {
						dao.register(user);
				...
				if (email.equals("admin@admin.com") && password.equals("admin")) {
					...
					if (email == null || email.trim().length() == 0 || password == null
						...
						resp.sendRedirect(
								"login.jsp?msg=Error! All the fields are mandatory. Please provide the details");
					} else if (user != null) {
						req.getSession().setAttribute("user", user);
						resp.sendRedirect("welcome.jsp?msg=Successfully logged in as " + user.getFname() + " "
								+ user.getLname() + " (" + user.getRole() + ") ");
